...
class TaskExecutor:
    ...
    def run(self, task: str) -> str:
        agent = create_react_agent(self.llm, self.tools)    # function calling & tools calling
        result = agent.invoke(
            {
                "messages": [
                    (
...
class SinglePathPlanGeneration:
    def __init__(self, llm: ChatOpenAI):
        ...
        self.task_exucutor = TaskExecutor(llm=llm)
    ...
    def _execute_task(self, state: SinglePathPlanGenerationState) -> dict[str, Any]:
        ...
        result = self.task_exucutor.run(task=current_task)
        return {
            "results": [result],
