...
class ReflectionJudgment(BaseModel):
    ...
    reasons: list[str] = Field(description="タスクの実行結果の適切性とそれに対する自信度について、判断に至った理由を簡潔に列挙してください。")  # ？？？为什么这是个列表


class Reflection(BaseModel):
    id: str = Field(description="リフレクション内容に一意性を与えるためのID")   # an unique id for each `reflection` object
    task: str = Field(description="ユーザーから与えられたタスクの内容")
    reflection: str = Field(description="このタスクに取り組んだ際のあなたの思考プロセスを振り返ってください。\
                            何か改善できる点ありましたか?次に同様のタスクに取り組む際に、より良い結果を出すための教訓を2~3文程度で簡潔に述べてください。")
...
class ReflectionManager:
    ...
    def load_reflections(self):
        if os.path.exists(self.file_path):
            ...
            if self.reflections:
                ...
                # Create an empty FAISS flat (exact) L2 index(vector-search space ) with vector dimension d. (compute L2 distance; top-k)
                self.index = faiss.IndexFlatL2(len(embeddings[0]))
                # Add all embeddings (shape: (n, d)) to the index; FAISS requires float32.
    ...
    def save_reflection(self, reflection: Reflection) -> str:
        ...
        self.index.add(np.array([embedding]).astype("float32")) # type: ignore
...
class TaskReflector:
    ...
    def run(self, task: str, result: str) -> Reflection:
        ...
        # Try `invoke_chain()` 5 times. If errors occur, try again.
        @retry(tries=5)
        def invoke_chain() -> Reflection:
            return chain.invoke({"task": task, "result": result}) # type: ignore
